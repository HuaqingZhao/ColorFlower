...
namespace ColorFlower
{
    public class POP3
    {
        ...
        /// <returns></returns>
        public string Decode(string[] strss)  //=?utf-8?B?5Lqy54ix55qEIDExNzg3NzIxMjNxcWNv?=
        {
            //获取标题的编码方式
        ...
        public List<MailMessage> GetNewMessages()
        {
            ...
            try
            {
                ...
                for (int n = 1; n < newcount + 1; n++)
                {
                    ...
                    msg.Headers.Set("Date", GetSentDateTime(msglines).ToString());
        ...
        private static DateTime GetSentDateTime(ArrayList msglines)
        {
            ...
            foreach (var item in msglines)
            {
                ...
                {
                    DateTime.TryParse(item.ToString().Substring(5), out date);
